...
class Line
  ...
  def incomplete?
    !corrupt? && !@open_chunks.empty?
  end

  def completion_score
    completed_by = @open_chunks.reverse.map{ |c| CodeSymbol.new(symbol: c.closed_by)}

    completion_score = 0
    completed_by.each do |completion_symbol|
      completion_score = (completion_score * 5) + completion_symbol.autocomplete_score
    end
